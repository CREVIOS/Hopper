<script lang="ts">
  ...
  import { VM_PLAN_INFO, type VmPlan, type Pod } from '$lib/types';
  import { api } from '$lib/api/client';
  import { invalidateAll } from '$app/navigation';

  let { data }: { data: { pods: Pod[] } } = $props();

  let selectedPlan: VmPlan = $state('small');
  let creating = $state(false);

  async function createPod() {
    creating = true;
    try {
      await api.post('/pods/', { plan: selectedPlan });
      await invalidateAll();
    } catch (e) {
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold">Virtual Machines</h1>
    <div class="flex items-center gap-3">
      <select bind:value={selectedPlan} class="rounded border px-3 py-2">
        {#each Object.entries(VM_PLAN_INFO) as [plan, info]}
          <option value={plan}>{plan} — {info.cpu}, {info.memory} ({info.rate} cr/hr)</option>
        {/each}
      </select>
      <button
        onclick={createPod}
  ...
  <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
    {#each data.pods as pod (pod.id)}
      <div class="relative">
        ...
        {#if pod.state !== 'terminated' && pod.state !== 'stopping' && pod.state !== 'failed'}
          <button
    ...
    {:else}
      <p class="text-gray-500">No VMs yet. Launch one to get started.</p>
    {/each}
  </div>
</div>
