...
include \texttt{test\_credit\_ledger\_invariants.py},
\texttt{test\_nats\_event\_flow.py}, \texttt{test\_keycloak\_integration.py},
\texttt{test\_pods\_api.py}, \texttt{test\_auth\_api.py}, and 13 others.
...
\item[\textbf{TC-1 --- Billing consumer: insufficient-credit grace logic}]
  ...
  credit deduction raises \texttt{Insufficient\-Credits}, the consumer invokes
  the grace-period logic rather than nakking or crashing.
  Test: \texttt{test\_handle\_billing\_deducted\_runs\_grace\allowbreak{}\_logic\_on\_insufficient\_credits}.
...
\item[\textbf{TC-2 --- Credit service: double-entry balance invariant}]
  ...
  bookkeeping.
  Test: \texttt{test\_add\_credits\_creates\_transfer\allowbreak{}\_and\_balanced\_entries}.
...
\item[\textbf{TC-3 --- Orchestrator: pod lifecycle state transitions}]
  ...
  \texttt{TestPodLifecycle\-ValidTransitions} asserts the legal path REQUESTED
  $\to$ RUNNING $\to$ TERMINATED, and
  \texttt{TestPodLifecycle\-RejectsInvalidTransition} confirms that
...
\item[\textbf{TC-4 --- Orchestrator: VM plan billing rates}]
  ...
  credits/hour. \texttt{TestPartialHour\-BillingUsesPerMinuteRate} verifies
